...
for c = 1:numel(plotfields.bin)
    for nbin = 1:numel(dps.bin)
        ...
        im3d.bright = mask.*(dps.bin{1,nbin}.f.norm_k_mclu);
        ...
        clear im3d
        im3d = (dps.bin{1,nbin}.f.norm_k_mclu).*dps.bin{1,nbin}.(plotfields.bin{c});
        nii_fn = mdm_nii_write(im3d, tmp_fn, dps.nii_h, 0);        
    end
end
...
clear im3d
im3d.r = (dps.bin{1,1}.f.norm_k_mclu);
im3d.g = (dps.bin{1,2}.f.norm_k_mclu);
im3d.b = (dps.bin{1,3}.f.norm_k_mclu);
c_norm = zeros([3 msf_size(im3d.r,3)]);
c_norm(1,:,:,:) = im3d.r;
c_norm(2,:,:,:) = im3d.g;
c_norm(3,:,:,:) = im3d.b;
