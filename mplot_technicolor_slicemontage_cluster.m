function mplot_technicolor_slicemontage_cluster(method, dps, slices_path, clim, opt,slice_select,slmoNrow,slmol_v,slmob_v)
...
% for nk = 2
    ...
    for c = 1:numel(plotfields.bin)
        for nbin = 1:Nbins
            ...
            im3d.bright = mask.*(dps.bin{nbin}.f.norm_k_mclu);
    ...
    clear im3d
    im3d.r = (dps.bin{1}.f.norm_k_mclu);
    im3d.g = (dps.bin{2}.f.norm_k_mclu);
    im3d.b = (dps.bin{3}.f.norm_k_mclu);
%    im3d.bright = mask.*((dps.bin{1}.f.norm_k_mclu) + (dps.bin{2}.f.norm_k_mclu) + (dps.bin{3}.f.norm_k_mclu));
%    im3d.bright = mask;
%     im3d.bright
    im2d = zeros(sz(2),sz(1),3);
    im2d(:,:,1) = im3d.r(:,:,nk)';
...
%    im2d = im2d./repmat(max(im2d,[],3),[1 1 3]).*repmat(im3d.bright(:,:,nk)',[1 1 3]);
    ...
    print(fullfile(slices_path,fig_name),'-loose','-dsvg')
